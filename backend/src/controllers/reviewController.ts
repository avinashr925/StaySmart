...
export const createReview = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  // 2.5) Check if user has a confirmed stay reservation that has already started or completed
  const hasValidBooking = await Booking.exists({
    user: authorId,
    listing: listingId,
    status: "Confirmed",
  ...
  if (!hasValidBooking) {
    return next(
      new AppError("You can only review properties where you have a confirmed, active or completed reservation.", 403)
    );
  }
